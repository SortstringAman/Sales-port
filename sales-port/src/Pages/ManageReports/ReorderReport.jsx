...
import { inventoryData, reorderReportData } from './data';
 ...
 const columns = useMemo(
  () => [
        ...
        Cell: ({ value, row }) => (
          ...
          >
            <p style={{ margin: 0, fontWeight: 500,fontSize:'14px', color: '#7F56DA', textAlign: 'start' }}>
              {value} <br />
              <span style={{ fontSize: '10px', color: '#ADAEBC' ,fontWeight: 700}}>({row.original.sku_code})</span>
            </p>
          </div>
        ),
      },
    {
      ...
      accessor: 'UOM',
      disableSortBy: true,
      Cell: ({ value }) => <p style={{ textAlign: 'start', margin: 0, color: '#222F3E' ,fontSize: '14px'  ,fontWeight: 400}}>{value || '-'}</p>,
    },
    {
      ...
      disableSortBy: true,
      Cell: ({ value }) => (
        <p style={{ textAlign: 'start', fontWeight: '600',fontSize:'14px', margin: 0, color: '#222F3E' }}>
    ...
    },
    {
      ...
      disableSortBy: true,
      Cell: ({ value }) => <p style={{ textAlign: 'start', margin: 0, color: '#222F3E' }}>{value || '-'}</p>,
    },
    {
      Header: 'Min Level',
      accessor: 'min_level',
    ...
    {
      Header: 'Last Stock In Date',
      accessor: 'last_stock_in_date',
      disableSortBy: true,
      Cell: ({ value }) => <p style={{ textAlign: 'start', margin: 0, color: '#222F3E' }}>{value || '-'}</p>,
    },
    {
      Header: 'Last Stock Out Date',
      accessor: 'last_stock_out_date',
      disableSortBy: true,
      Cell: ({ value }) => <p style={{ textAlign: 'start', margin: 0, color: '#222F3E' }}>{value || '-'}</p>,
    },
    {
      Header: 'Suggested Action',
      accessor: 'suggested_action',
      disableSortBy: true,
      Cell: ({ value }) => <p style={{ textAlign: 'start', margin: 0, color: '#FF9A04',fontWeight:600,fontSize:'14px' }}>{value || '-'}</p>,
    },
    {
    ...
    <>
      <div className="dashboard">
        ...
        <div style={{ display: "flex", justifyContent: 'space-between', marginBottom: '28px' }}>
          <div >
            <h2 className='main-heading'>Reorder Alert Report</h2>
          </div>
