  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: 'img',
        Cell: ({ value }) => {
          const imageUrl = value ? value : image;
          return (
            <img
              src={imageUrl}
              alt="Profile"
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div>
            ...
            {/* <p style={{ textAlign: 'left', margin: 0 }}>({row.original.year})</p> */}
          </div>
        ),
      },
      {
      ...
      //   accessor: 'contacts',
      //   Cell: ({ row }) => {
      //     const contact = row.original.contacts.find(c => c.contact_type === 'Mobile');
      ...
      // },
      {
        ...
        accessor: 'contacts',
        Cell: ({ row }) => {
          const contact = row.original.contacts.find(c => c.contact_type === 'Mobile') || row.original.contacts[0];
          const number = contact?.number || 'N/A';
          const isWhatsapp = contact?.is_available_on_whatsapp;
        ...
        Cell: ({ row }) => (
          <div >
            <div style={{ display: 'flex', alignItems: 'center', gap: "15px" }}>
              {row.original.provisional_fees_status ? <img src={adstatus} data-tooltip-id="admission-tip" data-tooltip-content="Provisional Admission Confirmed"></img> : <img src={adstatusred}
                data-tooltip-id="admission-tip" data-tooltip-content="Provisional Admission Pending"></img>}
              {row.original.registration_status ? <img src={adstatus} data-tooltip-id="admission-tip" data-tooltip-content="Admission Confirmed"></img> : <img src={adstatusred} data-tooltip-id="admission-tip" data-tooltip-content="Admission Pending"></img>}
              {row.original.fees_payment_status ? <img src={adstatus} data-tooltip-id="admission-tip" data-tooltip-content="Fee Paid"></img> : <img src={adstatusred} data-tooltip-id="admission-tip" data-tooltip-content="Fee Pending"></img>}
              <Tooltip id="admission-tip" place="top" />
            </div>
          </div>
        ),
      },
      ...
      //       </div>
      //       {/* <img src={edit} style={{ cursor: 'pointer' }} alt="Edit" onClick={() => handleEditClick(row.original.id,row)} /> */}
      //       <img src={confirmadicon} style={{ cursor: 'pointer' }} alt="Edit" onClick={() => handleEditClick(row.original.id,row)} />
      //     </div>
      //   ),
      // },
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            <div style={{ display: 'flex', gap: '10px' }}
              data-tooltip-id="admission-tip" data-tooltip-content="Disable/Enable">
              {row.original.permanent_registration_number && (<div className="form-check form-switch custom-switch">
                <input
              ...
              {!row.original.permanent_registration_number && (
                <img
                  ...
                  onClick={() => handleregisClick(row.original.id, row)}
                  data-tooltip-id="admission-tip" data-tooltip-content="Confirm Admission"
                />
              )}
              {row.original.permanent_registration_number && <img src={edit} style={{ cursor: 'pointer' }} alt="Edit" onClick={() => handleEditClick(row.original.id, row)}
                ...
                row.original.permanent_registration_number ? (
                  <img
                    ...
                    onClick={() => {
                      ...
                      handlegenerateIdcard(row.original.id);
                      setidcardmodal(true);
                    }}
                    data-tooltip-id="admission-tip" data-tooltip-content="Generate ID Card"
                  />
                ) : null
              }
            </div>
      ...
      // {
      //   Header: <img src={setting} alt="Settings" />,
      //   accessor: 'setting',
      //   Cell: ({ row }) =>
    ...
    <>
      <div className="dashboard">
        ...
        <div style={{ display: "flex", justifyContent: 'space-between' }}>
          <div >
            <h2 className='main-heading'>Students Dashboard</h2>
          </div>
          <div >
            <a
              href="/admissionform"
      ...
      <div>
        <AddNewStudentModal isOpen={isModalOpen} onClose={closeModal} setsubmit={setsubmit}
          basicdata={basicdata} setbasicdata={setbasicdata}
          academicdata={academicdata} setacademicdata={setacademicdata} docdata={docdata} setdocdata={setdocdata} officialdata={officialdata} setofficialdata={setofficialdata} setIsEditMode={setIsEditMode} isEditMode={isEditMode} selectedUserId={selectedUserId}
          registrationnumforhead={registrationnumforhead} mainnamehead={mainnamehead} setgeneratedregno={setgeneratedregno} regMode={regMode} currentStep={currentStep} setCurrentStep={setCurrentStep} />
      </div>
      {idcardmodal ? <GenerateIDModal isOpen={openModal} onClose={closeModal} selectedOrgDetails={selectedOrgDetails} setCapturedPhoto={setCapturedPhoto} capturedPhoto={capturedPhoto}
        setShowEditor={setShowEditor} setFingerprints={setFingerprints} fingerprints={fingerprints}
        setimgurl={setimgurl} setidcardmodal={setidcardmodal} setidcarddata={setidcarddata} idcarddata={idcarddata}
      /> : ''}
      {showEditor && (
        <PhotoEditor capturedPhoto={capturedPhoto} setCapturedPhoto={setCapturedPhoto} setShowEditor={setShowEditor} setshowvaliditymodal={setshowvaliditymodal} setpaymentmodal={setpaymentmodal}
          imgurl={imgurl} setimgurl={setimgurl} setFingerprints={setFingerprints} selectedOrgDetails={selectedOrgDetails} fingerprints={fingerprints} />
      )}
      {submit === true && regMode ? <SuccessfulPopup mainheading={'Registration Completed Successfully!'} submitclose={submitclose} generatedregno={generatedregno} /> : ''}
      {submit === true && !regMode ?
        <div className="modal-overlay">
          <div className="modal-content formlinkmodal" style={{ position: 'relative' }}>
            <div className="modal-header-content" style={{ display: 'flex', justifyContent: 'end' }}>
              <button className="close-btn" onClick={() => { setsubmit(false) }}><img src={close}></img></button>
            </div>
